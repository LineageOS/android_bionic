 ...
 * Strings are *not* terminated by NULL, but by whitespace (isspace).
		...
		while (name < eol) {
			...
			if (len < MAX_HOSTLEN) {
				...
				if (c_alloc <= hcfile.c_len) {
					c_alloc += 256;
					hcfile.c_data = realloc(hcfile.c_data, c_alloc * sizeof(struct hcent));
				}
...
/*
 * Caching version of gethostbyname.
 *
 ...
 * will not.
 */
int hc_gethostbyname(const char *host, int af, struct hostent *hp,
		     char *buf, size_t buflen)
{
	...
	do {
		...
		if (inet_pton(af, addrstr, &addr) == 1) {
			...
			if (naddrs == 0) {
				...
				HENT_SCOPY(hp->h_name, namestr, buf, buflen);
			}
			HENT_COPY(addr_ptrs[naddrs], addr, addrlen, buf, buflen);
			++naddrs;
			if (naddrs >= MAXADDRS)
				goto nospc;
			for (name = hcfile.h_data + cur->name; name; name = _hcnextname(name)) {
				...
				HENT_SCOPY(aliases[naliases], namestr, buf, buflen);
				++naliases;
	...
	HENT_ARRAY(hp->h_aliases, naliases, buf, buflen);
	for (n = 0; n < naliases; ++n) {
		hp->h_aliases[n] = aliases[n];
	}
	hp->h_addrtype = af;
	hp->h_length = addrlen;
	HENT_ARRAY(hp->h_addr_list, naddrs, buf, buflen);
	for (n = 0; n < naddrs; ++n) {
		hp->h_addr_list[n] = addr_ptrs[n];
	}
